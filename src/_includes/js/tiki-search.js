...
(function (document) {
  ...
  const showFilterWrap  = document.createElement(`div`);
  const showFilter      = document.createElement(`input`);
  const showFilterLabel = document.createElement(`label`);
  let str = `Show filters`;

  showFilterWrap.setAttribute(`id`, `search-filter-toggle-wrapper`);
  showFilter.setAttribute(`type`, `checkbox`);
  showFilter.setAttribute(`id`, `search-filter-toggle`);
  showFilterLabel.setAttribute(`for`, `search-filter-toggle`);
  showFilterLabel.textContent = str;

  form.setAttribute(`class`, `field`);
  form.insertBefore(label, form.children[0]);
  form.appendChild(hint);
  results.prepend(showFilterWrap);
  showFilterWrap.append(showFilter);
  showFilterWrap.append(showFilterLabel);
  ...
  showFilter.addEventListener(
    `change`,
    (e) => {
      const filters = document.querySelector(`.pagefind-ui__filter-panel`);
      const label   = document.querySelector(`[for="search-filter-toggle"]`);

      if (e.target.checked) {
        filters.classList.add(`open`);
        let str = `Hide filters`;
